...
dataset_name = "vera365/lexica_dataset"
...
multimodal_model = anymodal.MultiModalModel(
    ...
    prompt_text="Caption the given image:")

if not os.path.exists("lexicaption"):
    os.makedirs("lexicaption")

snapshot_download("AnyModal/lexicaption-Llama-3.2-1B", local_dir="lexicaption")

multimodal_model._load_model('lexicaption')
...
for _ in range(5):
    ...
    with open(f"temp/image_{sample_idx}_caption.txt", "w") as f:
        f.write(f"Actual Prompt: {caption}\n")
        f.write(f"Generated Prompt: {generated_caption}\n")
