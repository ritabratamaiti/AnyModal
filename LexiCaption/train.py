...
for epoch in range(num_epochs):
    ...
    with torch.no_grad():
        ...
        for _ in range(5):
            ...
            print("Actual Prompt: ", sample['text'])
            with torch.autocast(device_type='cuda', dtype=torch.float16):
                print("Generated Prompt: ", multimodal_model.generate(sample['input'], max_new_tokens=120))
            
    multimodal_model.train()
    os.makedirs(f"lexicaption_{epoch+1}", exist_ok=True)
    multimodal_model._save_model(f"lexicaption_{epoch+1}")
